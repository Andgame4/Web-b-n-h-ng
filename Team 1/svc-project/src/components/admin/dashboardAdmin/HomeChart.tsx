    ...
    const [weekState,setWeekState] = useState<string[]>()
       ...
       useEffect(() => {
         const getProduct = async () => {
          ...
          var dayTime = orderData?.map((item: OrderDay) => week.push(moment(item.date).format('DD-MM')));
         };
         getProduct();
       }, [time]);
      ...
      const [timeGapBG, setTimeGapBG] = useState<BackgroundColor>({
        ...
        year: '',
            ...
            const dataDayTime = week;
             console.log('week là: ', dataDayTime);
            orderData?.map((item: OrderMonth) => month.push((item.month)?.toString()));  
            const dataMonthTime = month;
           console.log('month là: ', dataMonthTime);
